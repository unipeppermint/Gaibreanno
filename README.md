...
- Deck 页新增 YOUR EDGE：按当前草稿统计进攻牌、防御牌和功能牌；Guard / Pressure / Burst 可切换对应战术建议，建议随换牌重新计算，不直接修改卡牌效果。
- Collection 页新增 RISK / REWARD：以现有战斗规则比较 Present 即时效果与 Future 延迟效果，并说明等待、能量上限、首次攻击加倍、成长重置等代价。
- Rewind 明确标记 Future 不可用；Gear Guard 明确说明首次攻击加倍不等于护盾加倍。数值为基础值，实际伤害仍受敌方机制影响。
- 原配牌、保存、卡牌获取和搭配建议保留。新增内容为战术展示，没有额外扣费、隐藏胜率或新的随机结算。
